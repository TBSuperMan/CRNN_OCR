...
parser.add_argument('--resume', type=str, default="/home/gmn/crnn/checkpoint.pth.tar")
...
#日志
logger=utils.get_logger(osp.join("logs", "CRNN.logs"))
...
if opt.cuda:
    crnn.cuda()
    crnn = torch.nn.DataParallel(crnn, device_ids=[0,1,2,3])
...
def val(net, dataset, criterion, max_iter=100):
    ...
    data_loader = torch.utils.data.DataLoader(
        dataset, drop_last=True,shuffle=True, batch_size=opt.batchSize, num_workers=int(opt.workers))
    ...
    for k in range(len(train_iter)):
        ...
        if k % 1000  == 0:
            ...
            save_checkpoint({
                    ...
                    'epoch': epoch,
                    'best_res': acc_max,
                }, flag, fpath=osp.join("/home/gmn/crnn", 'checkpoint.pth.tar'))
